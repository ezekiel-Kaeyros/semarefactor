export class HelperMethod {
    ...
    static timeStringToMinutes(time: string) {
    ...
    static ajouterMinutes(date: Date, minutes: number) {
    ...
    static genererPlagesHoraires(dateStr: string, heureDebut: string, heureFin: string, dureeCreneau: number, typeFormat: number) {
        ...
        while (creneauActuel < finDate) {
            ...
            if (creneauFin > finDate) break;
            if(typeFormat == 1)plagesHoraires.push(`${creneauActuel.toTimeString().substr(0, 5)}`);
            else plagesHoraires.push(`${creneauActuel.toTimeString().substr(0, 5)} - ${creneauFin.toTimeString().substr(0, 5)}`);
    ...
    static ajouterJours(date: Date, jours: number) {
    ...
    static genererPlageDeDates(dateDebutStr: string, dureeJours: number, daysOfWeek: Array<string>) {
    ...
    static estDansPlageHoraire(date: Date, heureDebut: string, heureFin: string) {
